...
class UserDetailsSerializer(serializers.ModelSerializer):
    ...
    def validate_password(self,value):
        if not re.match(r'^(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*(),.?":{}|<>]).{8,}$',value):
            raise serializers.ValidationError("It must contain at least one uppercase letter, one lowercase letter, one digit, and one special character and length should be 8")
